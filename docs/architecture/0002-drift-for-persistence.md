...
**Status:** Proposed · **Date:** 2026-09-01 · **Supersedes the persistence clause of** [ADR 0001](0001-local-first-flutter.md)
...
```
lib/core/database/database_platform.dart          # driftDatabase() replaces the conditional export
...
`database_platform.dart` earned its place under sqflite, where three packages had to be reconciled by hand. `driftDatabase()` is that same reconciliation, maintained upstream. The `if (kIsWeb)` ban in CLAUDE.md still holds — we are removing the branch, not moving it into app code.
...
/// The per-platform executor for [config]'s flavor.
///
/// On native this is a background isolate holding the SQLite connection, so a
/// full-catalogue query never blocks the frame. On web it is sqlite3 compiled
/// to WebAssembly, running in a worker, persisted to OPFS where the browser
/// allows it and IndexedDB otherwise.
DatabaseConnection openConnection(AppConfig config) => driftDatabase(
  name: config.databaseName,
  native: DriftNativeOptions(
    // Application support, not documents: the catalogue is app-managed state,
    // not a file the user is invited to reorganise. drift_flutter defaults to
    // documents, so this override is load-bearing.
    databasePath: () async {
      final directory = await getApplicationSupportDirectory();
      if (!directory.existsSync()) await directory.create(recursive: true);
      return p.join(directory.path, '${config.databaseName}.sqlite');
    },
  ),
  web: DriftWebOptions(
    ...
    onResult: (result) {
      if (result.missingFeatures.isNotEmpty) {
        AppLogger.warn(
          'Web storage fell back to ${result.chosenImplementation}; '
...
`AppConfig.databaseFileName` becomes `databaseName` (`khulla_dev` / `khulla`, no extension) — drift appends `.sqlite` on native and uses the bare name as the OPFS/IndexedDB key on web, so one field still isolates the flavors on every platform.

The `onResult` callback is not decoration. On web, drift silently degrades to `inMemory` when the browser offers nothing persistent — a librarian would enter a day of circulation and lose it on refresh with no error anywhere. Log it now; surface it in the UI when the web build gets real use.
...
@lazySingleton
@DriftDatabase(tables: [Titles, Copies, Members, Loans, Fines])
class AppDatabase extends _$AppDatabase {
  AppDatabase(AppConfig config) : super(openConnection(config));
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    beforeOpen: (details) async {
      // SQLite defaults foreign keys OFF, per connection. Without this a
      // delete orphans every loan pointing at the record instead of failing.
      // Must run outside a transaction, which is why it lives here.
  ...
  /// first query.
  ///
  /// Drift opens lazily. `bootstrap` needs the failure *before* the first
  /// frame so it can show [StartupFailureApp] instead of a screen that throws
  /// once the operator taps something.
...
extension _Migrations on GeneratedDatabase {
  OnUpgrade get _upgrade => (m, from, to) async {
    // Drift calls onUpgrade for downgrades too, with from > to. sqflite had a
    // dedicated onDowngrade hook; here the guard is ours to write. ADR 0001
    // is explicit that this app refuses rather than deletes — it holds a
    // library's only copy of its records.
    if (from > to) {
      throw const DatabaseUnavailableException(
        'This library file was created by a newer version of Khulla.',
      );
    }
    await stepByStep(/* from1To2: …, generated */)(m, from, to);
  };
}
```
...
- **`stepByStep` is pulled out into an extension on `GeneratedDatabase`, not read off `this`.** Referring to the live database inside a migration step silently uses the *current* schema instead of that step's snapshot, which is how a migration passes in development and corrupts a real upgrade. Drift's own docs lead with this warning.
...
`guardDatabase` and `AppException` survive; only the driver types change.

```dart
// lib/core/error/guard.dart — the catch clauses
} on AppException {
  rethrow;
} on DriftRemoteException catch (error, stackTrace) {
  // Native runs the database in a background isolate, so a SqliteException
  // arrives wrapped. Unwrap before classifying, or every constraint violation
  // degrades to UnknownException.
  final cause = error.remoteCause;
  if (cause is SqliteException) throw AppException.fromSqlite(cause);
  ...
} on SqliteException catch (error, stackTrace) {
  throw AppException.fromSqlite(error);
} on InvalidDataException catch (error, stackTrace) {
  // Drift rejected the row before it reached SQLite — a length constraint,
  // a null in a non-nullable column.
  throw InvalidInputException(error.toString());
}
```

`AppException.fromDatabaseException` becomes `AppException.fromSqlite`, classifying on `SqliteException.extendedResultCode` instead of sqflite's `isUniqueConstraintError()` helpers:

| Extended code | Constant | Maps to |
| --- | --- | --- |
...
Note 787 moving to `ConflictException` rather than the old catch-all: under sqflite there was no FK helper to test, so the case was invisible. "That record is still referenced" is exactly the `ConflictException` docstring, and it is the error a librarian hits deleting an author who still has titles.
...
Each step ends green — `make analyze` clean and `make test` passing — so the branch is never mid-rewrite.

1. **Swap dependencies.** pubspec edits, `make bootstrap`. Verify the sqlite3 build hook resolves on Windows, Linux and web before writing anything on top of it. *This is the step most likely to surprise; do it alone.*
2. **Web assets.** New `db-web` target, fetch both files, delete `sqflite_sw.js`.
3. **Connection and database class.** `connection.dart`, `app_database.dart` with an empty `@DriftDatabase(tables: [])`, `AppConfig.databaseName`. `warmUp()` wired into `bootstrap` in place of `AppDatabase.open()`. Delete the three `database_platform*` files and both migration files. `build.yaml`. `make build`.
4. **Errors.** `AppException.fromSqlite`, `guardDatabase` catch clauses, drop the `sqflite_common` import from `app_exception.dart`. Unit-test the extended-code mapping — it is pure logic and cheap to pin.
5. **First table.** Whichever `catalog` sub-feature lands first: table class, `MoneyConverter` if it carries an amount, `make migrate` to record `drift_schema_v1.json`, first data source on top of it.
6. **Docs.** CLAUDE.md Database and Money sections, the Definition of done checklist, `.cursor/rules/architecture.mdc`, and a note on ADR 0001 that its persistence clause is superseded here.

Steps 1–4 are a self-contained branch with no behaviour change: the app opens an empty drift database instead of an empty sqflite one. Step 5 is the first feature branch that builds on it.
