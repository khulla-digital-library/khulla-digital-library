// Copyright (c) 2026 Khulla Digital Library contributors.
// SPDX-License-Identifier: MIT

/// Khulla design system: tokens, theme, and primitives.
///
/// `app_palette.dart` is intentionally not exported. Colors reach widgets
/// through `context.colorScheme` or `context.appColors`.
library;

export 'package:flutter/material.dart';

export 'src/extensions/build_context_extensions.dart';
export 'src/gallery/app_design_gallery.dart';
export 'src/gallery/app_gallery_controls.dart';
export 'src/gallery/app_gallery_data.dart';
export 'src/gallery/app_gallery_foundations.dart';
export 'src/gallery/app_gallery_section.dart';
export 'src/gallery/app_gallery_specimens.dart';
export 'src/gallery/app_gallery_states.dart';
export 'src/gallery/app_gallery_surfaces.dart';
export 'src/icons/app_icon.dart';
export 'src/icons/app_icon_spec.dart';
export 'src/icons/app_icons.dart';
export 'src/theme/app_borders.dart';
export 'src/theme/app_brand.dart';
export 'src/theme/app_brand_theme.dart';
export 'src/theme/app_breakpoints.dart';
export 'src/theme/app_colors.dart';
export 'src/theme/app_density.dart';
export 'src/theme/app_metrics.dart';
export 'src/theme/app_motion.dart';
export 'src/theme/app_radius.dart';
export 'src/theme/app_responsive_theme.dart';
export 'src/theme/app_scroll_behavior.dart';
export 'src/theme/app_shadows.dart';
export 'src/theme/app_spacing.dart';
export 'src/theme/app_text_styles.dart';
export 'src/theme/app_theme.dart';
export 'src/widgets/app_avatar.dart';
export 'src/widgets/app_bar_chart.dart';
export 'src/widgets/app_bottom_bar.dart';
export 'src/widgets/app_bottom_sheet.dart';
export 'src/widgets/app_breadcrumbs.dart';
export 'src/widgets/app_button.dart';
export 'src/widgets/app_button_size.dart';
export 'src/widgets/app_button_style.dart';
export 'src/widgets/app_button_variant.dart';
export 'src/widgets/app_card.dart';
export 'src/widgets/app_chart_data.dart';
export 'src/widgets/app_checkbox_field.dart';
export 'src/widgets/app_color_picker.dart';
export 'src/widgets/app_compact_quantity_control.dart';
export 'src/widgets/app_content_constraint.dart';
export 'src/widgets/app_dashed_border.dart';
export 'src/widgets/app_detail_row.dart';
export 'src/widgets/app_dialog.dart';
export 'src/widgets/app_dialog_close_chip.dart';
export 'src/widgets/app_dialog_shell.dart';
export 'src/widgets/app_donut_chart.dart';
export 'src/widgets/app_dropdown_field.dart';
export 'src/widgets/app_dropdown_field_control.dart';
export 'src/widgets/app_dropdown_menu.dart';
export 'src/widgets/app_dropdown_menu_item.dart';
export 'src/widgets/app_empty_view.dart';
export 'src/widgets/app_error_view.dart';
export 'src/widgets/app_feedback_variant.dart';
export 'src/widgets/app_field_affix.dart';
export 'src/widgets/app_field_label.dart';
export 'src/widgets/app_filter_chip.dart';
export 'src/widgets/app_form_modal.dart';
export 'src/widgets/app_form_row.dart';
export 'src/widgets/app_form_section.dart';
export 'src/widgets/app_icon_button.dart';
export 'src/widgets/app_legend_dot.dart';
export 'src/widgets/app_line_chart.dart';
export 'src/widgets/app_link_button.dart';
export 'src/widgets/app_menu_action.dart';
export 'src/widgets/app_menu_button.dart';
export 'src/widgets/app_nav_bar.dart';
export 'src/widgets/app_nav_destination.dart';
export 'src/widgets/app_nav_rail.dart';
export 'src/widgets/app_page_body.dart';
export 'src/widgets/app_pagination.dart';
export 'src/widgets/app_picker_field.dart';
export 'src/widgets/app_positive_int_formatter.dart';
export 'src/widgets/app_progress_bar.dart';
export 'src/widgets/app_quantity_field.dart';
export 'src/widgets/app_radio_field.dart';
export 'src/widgets/app_rail_badge.dart';
export 'src/widgets/app_rail_item.dart';
export 'src/widgets/app_rail_row_surface.dart';
export 'src/widgets/app_regular_quantity_control.dart';
export 'src/widgets/app_responsive_grid.dart';
export 'src/widgets/app_ripple.dart';
export 'src/widgets/app_search_field.dart';
export 'src/widgets/app_section_header.dart';
export 'src/widgets/app_segmented_control.dart';
export 'src/widgets/app_side_sheet.dart';
export 'src/widgets/app_skeleton.dart';
export 'src/widgets/app_sliding_number.dart';
export 'src/widgets/app_sliver_table.dart';
export 'src/widgets/app_spinner.dart';
export 'src/widgets/app_stat_strip.dart';
export 'src/widgets/app_stat_tile.dart';
export 'src/widgets/app_status_badge.dart';
export 'src/widgets/app_status_tone.dart';
export 'src/widgets/app_swatch_picker.dart';
export 'src/widgets/app_switch_field.dart';
export 'src/widgets/app_table.dart';
export 'src/widgets/app_table_column.dart';
export 'src/widgets/app_table_header.dart';
export 'src/widgets/app_table_row.dart';
export 'src/widgets/app_table_sort.dart';
export 'src/widgets/app_text_button.dart';
export 'src/widgets/app_text_field.dart';
export 'src/widgets/app_toolbar.dart';
export 'src/widgets/app_top_bar.dart';
export 'src/widgets/app_trend_pill.dart';
